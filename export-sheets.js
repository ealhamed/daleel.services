...
const servicesHeaders = [
  ...
  'category_ar', 'category_en', 'tags', 'penalty_ar',
...
// ═══ DETAILS TAB ═══
const detailsHeaders = ['slug', 'type', 'order', 'text_ar', 'text_en', 'amount', 'unit_ar']
const detailsRows = []
...
for (const s of services) {
  ...
  // Fees
  ;(s.fees || []).forEach((f, i) => {
    detailsRows.push([s.slug, 'fee', i + 1, f.label_ar, '', f.amount, f.unit_ar])
  })
  // Steps
  ...
  ;(s.troubles || []).forEach((t, i) => {
    detailsRows.push([s.slug, 'trouble', i + 1, t.question_ar, t.answer_ar, '', ''])
  })
}
...
console.log(`  → services.csv (${servicesRows.length} rows)`)
console.log(`  → details.csv  (${detailsRows.length} rows)`)
console.log('')
...
console.log('4. Create sheets.config.json:')
console.log('   {')
console.log('     "services_csv_url": "YOUR_SERVICES_CSV_URL",')
console.log('     "details_csv_url": "YOUR_DETAILS_CSV_URL"')
console.log('   }')
